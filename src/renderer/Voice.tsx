...
// @ts-ignore
import polusSvg from '../../static/maps/polus.svg';
import lo from 'lodash';
import {AudioComputationNode} from "./audio-graph";
import {AudioMap, ZonedAudioMap, Point,  Geometry} from "./map";
...
function calculateVoiceAudio(
    ...
    let panPos = state.gameState === GameState.LOBBY ? [other.x - me.x, other.y - me.y] : (vector == undefined ? [Number.NaN, Number.NaN] : [Math.cos(vector[0])*vector[1], Math.sin(vector[0])*vector[1]]);
    ...
    const bound = 999;
    panPos = [0, 1].map(i => panPos[i]).map(v => isNaN(v) ? bound : Math.min(bound, Math.max(-bound, v)));
    if (other.inVent) {
        gain.gain.value = 0;
        return;
    }
    ...
    if (
        gain.gain.value === 1 &&
        Math.sqrt(Math.pow(panPos[0], 2) + Math.pow(panPos[1], 2)) > 10
    ) {
        gain.gain.value = 0;
    }
}
...
const Voice: React.FC<VoiceProps> = function ({
    ...
    const [zonedMap, setZonedMap] = useState<ZonedAudioMap>();
    const mapName = "polus";
    ...
    useEffect(() => {
        async function parseMap() {
            return await fetch(polusSvg)
                .then(r => r.text())
                .then(text => new AudioMap(text))
                .then(map => new ZonedAudioMap(2.66 * 3, map))
                .then(map => setZonedMap(map))
        }
    ...
    const myPlayer = useMemo(() =>
            !gameState || !gameState.players ?
                undefined :
                gameState.players.find((p) => p.isLocal),
        [gameState.players]
    );
    ...
    const otherPlayers = useMemo(() => {
        ...
        let vectors : [Player, number, number][] = [];
        if(zonedMap != undefined && gameState.gameState == GameState.TASKS){
            const myPlayerPoint: Point = [myPlayer.x, myPlayer.y];
            const mapZone = zonedMap.getZoneForPoint(myPlayerPoint);
            if(mapZone != undefined && otherPlayers != undefined){
                const playersInRange = lo(otherPlayers)
                                .filter(p => !p.isDead && !p.disconnected)
                                .filter(p => Geometry.dist([p.x, p.y], myPlayerPoint) <= zonedMap.speakingRadius)
                                .value();
                const graph = mapZone.graph.clone();
                const visibleNodes = (player: Player) => lo(mapZone.graph.getNodes())
                    .filter(n => lo(mapZone.lines).filter(l => Geometry.intersects(l, [[player.x, player.y], n.value])).isEmpty())
                    .map(n => graph.getNodes()[n.id]);
                const myPlayerNode = graph.addNode(myPlayerPoint);
                visibleNodes(myPlayer).forEach(n => graph.addDirectedEdge(myPlayerNode, n, Geometry.dist(myPlayerPoint, n.value)));
                const otherPlayerNodes = new Map<Player, AudioComputationNode<Point>>();
                playersInRange.forEach(p => otherPlayerNodes.set(p, graph.addNode([p.x, p.y])));
                otherPlayerNodes.forEach((pNode, p) => {
                    visibleNodes(p).forEach(n => graph.addDirectedEdge(n, pNode, Geometry.dist(n.value, pNode.value)))
                    if(lo(mapZone.lines).filter(l => Geometry.intersects(l, [myPlayerPoint, pNode.value])).isEmpty()){
                        graph.addDirectedEdge(myPlayerNode, pNode, Geometry.dist(myPlayerPoint, pNode.value));
                    }
                });
                const spt = graph.computeSPT(myPlayerNode);
                vectors = lo(playersInRange)
                    .map(p => [p, otherPlayerNodes.get(p)] as [Player, AudioComputationNode<Point>]).compact()
                    .map(pair => [pair[0], spt.getPathsTo(pair[1])] as [Player, [AudioComputationNode<Point>[]]]).compact()
                    .map(pair => {
                        const lastLegPoint = lo(pair[1][0]).map(n => n.value).last();
                        // console.log(lastLegPoint);
                        // @ts-ignore
                        const rad = Math.atan2(lastLegPoint[1] - myPlayer.y, lastLegPoint[0] - myPlayer.x);
                        const d = lo(pair[1][0])
                            .map(n => n.value)
                            .reduceRight(
                                function (prev: [number, Point], p: Point): [number, Point] {
                                    return [prev[0] + Geometry.dist(prev[1], p), p];
                                },
                                [0, [myPlayer.x, myPlayer.y]]
                            )[0];
                        const ret:[Player, number, number] = [pair[0], rad, d];
                        return ret;
                    })
                    .filter(trip => trip[2] < zonedMap.speakingRadius)
                    .value();
            }
        }
        ...
        for (const player of otherPlayers) {
            ...
            if (audio) {
                const vector = lo(vectors).filter(trip => trip[0] == player).map<[number, number]>(trip => [trip[1], trip[2]]).first();
                calculateVoiceAudio(
                    gameState,
                    settingsRef.current,
                    myPlayer,
